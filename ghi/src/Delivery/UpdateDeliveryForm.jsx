...
import { useUpdateDeliveryMutation } from "../store/deliveryApi";
...
const DeliveryForm = () => {
  ...
  const [updateDelivery, result] = useUpdateDeliveryMutation();
  ...
  const handleProduceListChange = (event) => {
    const value = event.target.value;
    setProduceList(value);
  };
            ...
            <form onSubmit={handleSubmit} id="create-produce-form">
              <div>
                ...
                >
                  ...
                  {/* posting coming from state above. needs to be connect to get use all post query method line 22 */}
                  {posting.map((post) => {
                    return (
